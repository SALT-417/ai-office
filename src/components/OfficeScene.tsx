...
interface Props { mode: OfficeMode; selectedId: EmployeeId; progress: number; onSelect: (id: EmployeeId) => void; managerStatus: ManagerRequestStatus; assignedEmployeeIds: EmployeeId[] }

export function OfficeScene({ mode, selectedId, progress, onSelect, managerStatus, assignedEmployeeIds }: Props) {
  ...
  return <section className="scene-shell" aria-labelledby="scene-status">
    ...
    <div className="office-scene" data-mode={mode}>
      ...
      {employees.map((employee) => <EmployeeSprite key={employee.id} employee={employee} position={activeMode.positions[employee.id]} selected={employee.id === selectedId} mode={mode} activity={managerStatus === 'loading' && employee.id === 'ren' ? 'processing' : managerStatus === 'success' && assignedEmployeeIds.includes(employee.id) ? 'assigned' : undefined} onSelect={() => onSelect(employee.id)} />)}
