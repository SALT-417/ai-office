...
export function AppHeader({ progress, runtimeMode }: Props) {
  ...
  return <header className="app-header">
    ...
    <div className="runtime-header"><span className={isPublicDemo ? 'environment-badge demo' : 'environment-badge local'}><span aria-hidden="true">{isPublicDemo ? '◌' : '●'}</span>{isPublicDemo ? '公開サンプル・AI通信なし' : 'ローカルAI稼働'}</span><div className="header-progress"><div><span>PROJECT STATUS</span><strong>{progress}%</strong></div><ProgressBar value={progress} label={`プロジェクト全体の進捗 ${progress}%`} /></div></div>
  </header>;
}
