...
- **リアル／ミニチュア表示**：既存の実写風オフィスと、CSS・Reactで構成した軽量な2.5Dミニチュアオフィスを切り替えられます。ミニチュア背景は「AIオペレーションセンター × 親しみやすいITスタートアップ」をテーマに、5つの作業席、共有テーブル、ラウンジ、資料エリア、ステータスモニターを立体的に構成しています。顔や手足のある5名のミニキャラが巡回し、短い吹き出しで作業・相談・完了状態を表現します。移動は画面内でON/OFFでき、初期値は`prefers-reduced-motion`を尊重しつつ、利用者が明示的にONへ変更できます。背景と吹き出しはUI演出のみで、AI通信と安全設計は既存仕様のままです。
